...
const $from = $("#from");
const $to = $("#to");
const $algorithmChoosing = $("#algorithm-choosing");
const $dijkstraOption = $("#dijkstra-option");
const $primOption = $("#prim-option")
const $vertex = $("#vertex")
...
$dijkstraOption.hide();
$primOption.hide();
$btnRunAlgorithm.hide()

//chon prim hay dijsktra
$algorithmChoosing.on('change', function() {
    if(this.value == "dijkstra") {
        $dijkstraOption.show();
        $btnRunAlgorithm.show();
        $primOption.hide();
        activeAlgorithm = dijkstra;
    } else if(this.value == "prim") {
        $primOption.show();
        $btnRunAlgorithm.show();
...
function setShortestPathEdge(aName, bName) {
    let foundEdge = null;
    let pointsName;
    edges.every(edge => {
        pointsName = edge.name.split("_");
...
async function primAlgorithm(startVertex) {
    resetGraph();
    let ADDED;
    let x, y, xName, yName;
    let numberOfVertex = points.length;
    let numberSelectedEdge = 0;
    let pointsName = points.map(e => e.name)
    ADDED = new Array(points.length).fill(false);
    DISTANCES = new Array(points.length).fill(Number.POSITIVE_INFINITY);
    PARENTS = new Array(points.length).fill(-1);
    ADDED[startVertex] = true;
    console.log(points)
    console.log(edges)
    console.log("Edge : Weight\n")
    
    while(numberSelectedEdge < (numberOfVertex - 1)) {
        let min = Number.POSITIVE_INFINITY
        let edge;
        for(let i = 0; i < numberOfVertex; i++) {
            if(ADDED[i]) {
                for(let j = 0; j < numberOfVertex; j++) {
                    edge = getEdge(pointsName[i], pointsName[j])
                    if((!ADDED[j]) && edge) {
                        edge.processed = true;
                        updateCanvas()
                        showNotification(`Processing vertex ${pointsName[i]} to vertex ${pointsName[j]}`, animationSpeed)
                        await timer(animationSpeed)
                        if(min > edge.cost) {
                            min = pointsName[j].cost
                            xName = pointsName[i]
                            yName = pointsName[j]
                            x = i;
                            y = j;
                            ...
                            showNotification(`Found shortest path from vertex ${xName} to vertex ${yName}, distance: ${edge.cost}`, animationSpeed)
                        }
                    }
                }
            }
        }
        console.log(`${xName}_${yName}: Distance: ${getEdge(xName, yName).cost}`)
        updateCanvas()
        ADDED[y] = true
        numberSelectedEdge++
    }
...
function onRunClick() {
    if(!running) {
        if(activeAlgorithm == dijkstra) {
            runAlgorithm();
            return;
        } else if(activeAlgorithm == prim) {
            primAlgorithm($vertex.val().charCodeAt(0) - 65)
            return;
        }
    }
